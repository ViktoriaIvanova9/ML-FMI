...
def main():    
    ...
    steps = [('scaler', StandardScaler())] # it was not working when applying PCA in the pipeline
    pipeline = Pipeline(steps).fit_transform(samples)
    pca = PCA().fit(pipeline)

    pca_transformed = pca.transform(pipeline)

    print(f'Shape of PCA features: {np.shape(pca_transformed)}') # how to decide which features exactly to be in? 
                                                                 # At first glance in the barplot I would say 2 but is there exact number
                                                                 # on which to decide?
