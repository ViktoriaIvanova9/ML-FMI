...
def main():
    ...
    bagging = BaggingClassifier(estimator=DecisionTreeClassifier(random_state=1, min_samples_leaf=8), 
                                n_estimators=50, 
                                random_state=1,
                                oob_score=True).fit(X_train_prepared, y_train)


    print(f'Mean accuracy of aggregator on OOB instances: {round(np.mean(bagging.oob_score_), 2)}')
    print(f'Test set accuracy: {round(bagging.score(X_test_prepared, y_test), 2)}') # what better accuracy here means?
