...
def main():
    ...
    gradient_boosting_regressor = GradientBoostingRegressor(max_depth=4, 
                                                            n_estimators=200, 
                                                            random_state=2,
                                                            subsample=0.9,
                                                            max_features=0.75).fit(X_train_prepared, y_train)

    y_pred = gradient_boosting_regressor.predict(X_test_prepared)

    print(f'Test set RMSE: {round(root_mean_squared_error(y_pred, y_test), 2)}') # not the same RMSE
