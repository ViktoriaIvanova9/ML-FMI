...
def main():
    ...
    kf = KFold(n_splits=5, shuffle=True) # on some of the tasks I receive the right result when shuffle is not set to True
    dt = RandomizedSearchCV(DecisionTreeClassifier(random_state=1), param_grid, cv=kf).fit(X_train_prepared, y_train)

    y_pred = dt.predict(X_test_prepared)

    print(dt.best_params_)
    print(f'Test set ROC AUC: {round(roc_auc_score(y_test, y_pred), 2)}') # not the same, tried with Grid search also
